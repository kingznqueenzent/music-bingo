...
export function VinylSpinner({
  ...
  return (
    ...
    >
      <div
        className={`absolute inset-0 rounded-full shadow-[0_0_40px_rgba(0,255,255,0.25)] ${
      ...
      >
        <div className="absolute inset-[14%] rounded-full overflow-hidden border border-white/10 bg-[#1E1E1E]">
          {albumArtUrl ? (
            ...
            <div className="w-full h-full flex items-center justify-center text-[#00FFFF]/50 text-4xl">
              ♪
            </div>
          )}
        </div>
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[12%] h-[12%] rounded-full bg-[#00FFFF] shadow-[0_0_12px_#00FFFF]" />
      </div>
    </div>
  )
}
