    ...
    <span
      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-semibold tabular-nums border ${
        empty
          ...
          : count < 10
            ? 'border-yellow-500/35 text-yellow-300 bg-yellow-500/10'
            : 'border-[var(--lg-neon)]/35 text-[var(--lg-neon)] bg-[var(--lg-neon)]/10'
      }`}
    >
...
function ThemeActionCard({
  ...
  return (
    <Link
      href={href}
      className="rounded-xl border border-white/10 bg-black/20 px-4 py-3 font-medium text-white/90 hover:border-[var(--lg-neon)]/40 hover:bg-[var(--lg-neon)]/5 transition-colors min-h-12 inline-flex flex-col sm:flex-row sm:items-center gap-1.5 sm:gap-2 touch-manipulation"
    >
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold mb-2 text-white">Load songs into a theme</h2>
        ...
        <button
          ...
          disabled={loading}
          className="inline-flex items-center gap-2 min-h-11 rounded-xl border border-white/10 px-3 py-2 text-sm text-white/60 hover:border-[var(--lg-neon)]/35 hover:text-[var(--lg-neon)] touch-manipulation disabled:opacity-50"
        >
      ...
      <div>
        <p className="text-[10px] uppercase tracking-wider text-white/35 mb-2">YouTube import</p>
        <div className="flex flex-wrap gap-3 mb-3">
          ...
          <Link
            ...
            className="rounded-xl border border-white/10 bg-black/20 px-5 py-3 font-medium text-white/45 hover:border-white/20 hover:bg-white/5 transition-colors min-h-12 inline-flex items-center"
          >
            All themes…
          </Link>
        </div>
      ...
      <div>
        <p className="text-[10px] uppercase tracking-wider text-white/35 mb-2">MP3 / MP4 (Media)</p>
        <div className="flex flex-wrap gap-3 mb-3">
          {featured.map((t) => (
            <ThemeActionCard
              key={`media-${t.id}`}
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto overscroll-contain pr-1">
            {rest.map((t) => (
              <li key={t.id}>
                <Link
                  href={`/host/import-youtube?theme=${t.id}`}
                  className="flex items-center justify-between gap-2 rounded-lg border border-white/10 bg-black/20 px-3 py-2.5 min-h-11 hover:border-[var(--lg-neon)]/35 transition-colors"
                >
